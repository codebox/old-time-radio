...
export class NameParser {
    ...
    findBestParser(playlistId, metadata) {
    ...
    updateStatsOk(playlistId, title) {
    ...
    updateStatsFailed(playlistId, fileName, fallbackTitle) {
    ...
    matchingParserWithNonMatchingRegex(playlistId, metadata) {
        const fallbackName = metadata.title || metadata.name;
        this.updateStatsFailed(playlistId, metadata.name, fallbackName);
        return fallbackName;
    }
    noMatchingParser(playlistId, metadata) {
        const fallbackName = metadata.title || metadata.name;
        this.updateStatsFailed(playlistId, metadata.name, fallbackName);
        return fallbackName;
    }
    matchingParserWithDisplayName(playlistId, parser) {
        const displayName = parser.displayName;
        this.updateStatsOk(playlistId, displayName);
        return displayName;
    }
    matchingParserWithMatchingRegex(playlistId, parser, match) {
        const titlePart = this.applyTransforms(match.groups.title, parser.transforms?.title), datePart = this.applyTransforms(match.groups.date, parser.transforms?.date), numPart = this.applyTransforms(match.groups.num, parser.transforms?.num);
        let fullTitle = parser.showName || configHelper.getShowForPlaylistId(playlistId).name;
        if (numPart) {
    ...
    matchingParserWithNoRegex(playlistId, metadata) {
...
//# sourceMappingURL=nameParser.mjs.map