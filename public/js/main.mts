...
import { buildEventSource } from './events.mjs';
import {
    EVENT_NEW_MESSAGE,
    EVENT_SHOW_SUMMARY,
    EVENT_HIDE_SUMMARY,
    EVENT_CLEAR_SUMMARY,
    SUMMARY_LINK_CLICK,
    EVENT_AUDIO_TRACK_LOADED,
    EVENT_AUDIO_PLAY_STARTED,
    EVENT_AUDIO_TRACK_ENDED,
    EVENT_AUDIO_ERROR,
    EVENT_SLEEP_TIMER_CLICK,
    EVENT_SLEEP_TIMER_TICK,
    EVENT_SLEEP_TIMER_DONE,
...
import { buildStateMachine, STATE_LOADING_TRACK, STATE_PLAYING, STATE_IDLE, STATE_TUNING_IN, STATE_ERROR, STATE_GOING_TO_SLEEP, STATE_SLEEPING } from './stateMachine.mjs';
import { buildModel } from './model.mjs';
import { buildView } from './view.mjs';
import { buildService } from './service.mjs';
import { buildAudioPlayer } from './audioPlayer.mjs';
import { buildVisualiserDataFactory } from './visualiserData.mjs';
import { buildVisualiser } from './visualiser.mjs';
import { buildMessageManager } from './messageManager.mjs';
import { buildSummaryManager } from './summaryManager.mjs';
import { buildSleepTimer } from './sleepTimer.mjs';
...
window.onload = () => {
    ...
    function startSnowMachineIfAppropriate() {
        function getSnowIntensityForToday(day: number, month: number): number | undefined {
            if (month === 11) {
                if (day <= 25) {
                    return 1 - (25 - day) / 25;
                } else {
    ...
    messageManager.on(EVENT_NEW_MESSAGE).then(event => {
        const { text } = (event as Event & { data: { text: string } }).data;
        view.showMessage(text);
    });

    summaryManager.on(EVENT_SHOW_SUMMARY).then(event => {
        const summaryText = (event as Event & { data: string }).data;
    ...
    view.on(EVENT_SLEEP_TIMER_CLICK).then(event => {
        const minutes = (event as Event & { data: number }).data;
    ...
    view.on(EVENT_STATION_BUILDER_SHOW_CLICK).then(event => {
        const clickedShow = (event as Event & { data: StationBuilderShow }).data;
    ...
    view.on(EVENT_VISUALISER_BUTTON_CLICK).then(event => {
        const visualiserId = (event as Event & { data: string }).data;
    ...
    function getChannels(): Promise<Channel[]> {
        ...
        if (urlChannelCodes) {
            ...
            const channels: Channel[] = urlChannelCodes.split(',').map((code, i) => {
                return {
                    id: code as ChannelId,
                    name: `Channel ${i + 1}`,
                    userChannel: true
                };
            });
