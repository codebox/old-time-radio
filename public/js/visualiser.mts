...
import type { Visualiser, VisualiserDataFactory } from './types.mjs';
...
export function buildVisualiser(dataFactory: VisualiserDataFactory): Visualiser {
    ...
    function makeRgb(v: number): string {
        return `rgb(${v},${v},${v})`;
    }
    ...
    const phonograph = (() => {
        ...
        return () => {
            ...
            const offset = Math.PI * 2 * (now - startTs) * phonographConfig.offsetRate,
                ...
                snapshotData: { radius: number; startAngle: number; endAngle: number }[] = [],
    ...
    const spirograph = (() => {
        ...
        function buildAlphaCalc(loopConfig: { minAlpha: number; maxAlpha: number; offset: number }) {
            const { minAlpha, maxAlpha, offset } = loopConfig;
            return (age: number, value: number): number => {
                const f = (offset + t / alphaCycleRate + value * age) % 1;
                return minAlpha + (maxAlpha - minAlpha) * f;
            };
        }

        function drawHistory(cx: number, cy: number, minRadius: number, maxRadius: number, alphaCalc: (age: number, value: number) => number, angleDiff: number, initialDirection: number) {
            let age = 0;
            history.forEach(p => {
                ...
                p.forEach((value, i) => {
                    ...
                    const alpha = alphaCalc(age, value);
    ...
    const visualiserLookup: Record<string, () => void> = {
        "None": () => {},
    ...
    return {
        init(_elCanvas: HTMLCanvasElement) {
            elCanvas = _elCanvas;
