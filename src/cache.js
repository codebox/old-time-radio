...
function memoize(fn, name) {
    const resultCache = buildCache(name, values => fn(...values));
    return (...args) => {
        return resultCache.get(args);
    };
}

function buildCache(name, source) {
    const expiryIntervalSeconds = config.caches.expirySeconds[name] || 0;

    log.info(`Created cache ${name} with expiry interval ${expiryIntervalSeconds} seconds`);
    ...
    const disk = (() => {
        const cacheDir = path.join(config.caches.location, name);
